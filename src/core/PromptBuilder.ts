import { Backlog, BacklogTask } from './Pipeline';
import { ContextBundle } from './WorkspaceScanner';

export class PromptBuilder {
	public buildPrompt(task: BacklogTask, backlog: Backlog, contextBundle?: ContextBundle): string {
	...
	private findStoryForTask(taskId: string, backlog: Backlog): {
		storyId: string;
		storyTitle: string;
	...
	private getBuildCommands(contextBundle?: ContextBundle): string[] {
	...
	private guessFilesToTouch(task: BacklogTask, contextBundle?: ContextBundle): string[] {
		const guessed = new Set<string>();
		const relevantFiles = contextBundle?.topRelevantFiles ?? [];
		const lowerTitle = task.title.toLowerCase();

		const pinIfExists = (target: string) => {
